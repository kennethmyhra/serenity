...
WebIDL::ExceptionOr<JS::Handle<WebIDL::ArrayBufferView>> Crypto::get_random_values(JS::Handle<WebIDL::ArrayBufferView> array_buffer_view) const
{
    ...
    if (!array_buffer_view->is_typed_array_base())
    ...
    if (array_buffer_view->byte_length() > 65536)
    ...
    if (array_buffer_view->array_buffer()->is_detached())
    ...
    fill_with_random(array_buffer_view->array_buffer()->buffer());
