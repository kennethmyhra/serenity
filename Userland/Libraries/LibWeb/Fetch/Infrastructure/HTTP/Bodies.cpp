...
WebIDL::ExceptionOr<void> Body::incrementally_read(ProcessBodyChunkCallback process_body_chunk, ProcessEndOfBodyCallback process_end_of_body, ProcessBodyErrorCallback process_body_error, TaskDestination task_destination) const
{
    ...
    dbgln("m_stream->vm().execution_context_stack().size(): {}", m_stream->vm().execution_context_stack().size());
    ...
    TRY(incrementally_read_loop(reader.get<JS::NonnullGCPtr<Streams::ReadableStreamDefaultReader>>(), task_destination, move(process_body_chunk), move(process_end_of_body), move(process_body_error)));
...
{
    dbgln("IncrementalReadLoopReadRequest::on_chunk");
    ...
    if (!chunk.is_object() || !is<JS::Uint8Array>(chunk.as_object())) {
        continue_algorithm = [&] {
            m_process_body_error(WebIDL::DOMException::create(realm, "TypeError", "Chunk is not a Uint8Array"sv));
        };
    }
    // 3. Otherwise:
    else {
        ...
        continue_algorithm = [&] {
            ...
            m_body.incrementally_read_loop(m_reader, m_task_destination, move(m_process_body_chunk), move(m_process_end_of_body), move(m_process_body_error)).release_value_but_fixme_should_propagate_errors();
        };
    }
