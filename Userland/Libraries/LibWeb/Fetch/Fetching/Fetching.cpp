    ...
    Platform::EventLoopPlugin::the().deferred_invoke([&realm, &vm, &fetch_params, request, response, get_response = move(get_response)] {
        ...
        pending_response->when_loaded([&realm, &vm, &fetch_params, request, response, response_was_null = !response](JS::NonnullGCPtr<Infrastructure::Response> resolved_response) mutable {
            ...
            if (!request->integrity_metadata().is_empty()) {
                ...
                //    error.
                auto process_body_error = JS::create_heap_function(realm.heap(), [&realm, &vm, &fetch_params](JS::GCPtr<WebIDL::DOMException>) {
                ...
                auto process_body = JS::create_heap_function(realm.heap(), [&realm, request, response, &fetch_params, process_body_error](ByteBuffer bytes) {
                ...
                TRY_OR_IGNORE(response->body()->fully_read(realm, process_body, process_body_error, fetch_params.task_destination()));
            }
...
WebIDL::ExceptionOr<void> fetch_response_handover(JS::Realm& realm, Infrastructure::FetchParams const& fetch_params, Infrastructure::Response& response)
{
    ...
    if (fetch_params.algorithms()->process_response_consume_body()) {
        ...
        auto process_body = JS::create_heap_function(realm.heap(), [&fetch_params, &response](ByteBuffer bytes) {
            (fetch_params.algorithms()->process_response_consume_body())(response, bytes);
        });

        // 2. Let processBodyError be this step: run fetchParams’s process response consume body given response and
        //    failure.
        auto process_body_error = JS::create_heap_function(realm.heap(), [&fetch_params, &response](JS::GCPtr<WebIDL::DOMException>) {
        ...
        if (!internal_response->body()) {
            Infrastructure::queue_fetch_task(fetch_params.controller(), task_destination, [process_body]() {
