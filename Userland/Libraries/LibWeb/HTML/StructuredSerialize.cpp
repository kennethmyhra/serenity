...
enum ValueTag {
    ...
    // Following four u32s are the tag and the value
    Primitive,

    // Following two u32s are the double value.
    NumberPrimitive,

    // The BigIntPrimtitive is serialized as a string in base 10 representation.
...
public:
    ...
    WebIDL::ExceptionOr<void> serialize(JS::Value value)
    {
        if (value.is_undefined() || value.is_null() || value.is_boolean()) {
            m_serialized.append(ValueTag::Primitive);
            u64 const tag = value.tag();
            // Extract the value from encoded().
            u64 const val = value.is_boolean() ? static_cast<u64>(value.as_bool()) : value.encoded() & ~tag;
            // Append tag and value to the serialized structure.
            m_serialized.append(bit_cast<u32*>(&tag), 2);
            m_serialized.append(bit_cast<u32*>(&val), 2);
...
public:
    ...
    WebIDL::ExceptionOr<void> deserialize()
    {
        ...
        while (position < m_vector.size()) {
            ...
            case ValueTag::Primitive: {
                u32 tag_bits[2];
                tag_bits[0] = m_vector[position++];
                tag_bits[1] = m_vector[position++];
                u64 const tag = *bit_cast<u64*>(&tag_bits);

                u32 val_bits[2];
                val_bits[0] = m_vector[position++];
                val_bits[1] = m_vector[position++];
                u64 const val = *bit_cast<u64*>(&val_bits);

                auto value = tag == JS::BOOLEAN_TAG ? JS::Value { static_cast<bool>(val) } : JS::Value { tag, val };
